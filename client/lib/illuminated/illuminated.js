...
(function(cp){
  ...
  **/
  ...
  rendered onto it.
  **/
  cp.Light.prototype._getVisibleMaskCache = function (center, arcStart, objects) {
    // By default use a radial gradient based on the distance
    var d = Math.floor(this.distance);
    var hash = ""+d+arcStart;
    if (this.vismaskhash != hash) {
      this.vismaskhash = hash;
      var c = this._vismaskcache = createCanvasAnd2dContext('vm'+this.id, 2*d, 2*d);
      var g = c.ctx.createRadialGradient(center.x, center.y, 0, d, d, d*1.1);
      g.addColorStop( 0, 'rgba(0,0,0,0.5)' );
      g.addColorStop( 1, 'rgba(0,0,0,0)' );
      c.ctx.fillStyle = g;

      c.ctx.beginPath();
      c.ctx.arc(center.y, center.x, d, arcStart-Math.PI/7, arcStart-Math.PI/7 + Math.PI/3.5, false);
      c.ctx.lineTo(center.y, center.x);
      c.ctx.closePath();
      c.ctx.fill();
    }
  ...
  **/
  ...
  cp.Lamp.defaults = {
    ...
    **/
    ...
    /**
    The roughness of the oriented effect.f
    @property roughness
    @type Number
    @default 0
    **/
  ...
  **/
  ...
  will be rendered.
  **/
  cp.Lamp.prototype.mask = function (ctx, arcStart) {
    var center = this.center();
    var c = this._getVisibleMaskCache(center, arcStart);
    var orientationCenter = new cp.Vec2(Math.cos(this.angle), -Math.sin(this.angle)).mul(this.roughness*this.distance);
  ...
  **/
  cp.Lamp.prototype._getGradientCache = function (center, arcStart) {
    var hashcode = this._getHashCache();
    if (this._cacheHashcode == hashcode+arcStart) {
      return this._gcache;
    }
    this._cacheHashcode = hashcode+arcStart;
    ...
    cache.ctx.beginPath();
    cache.ctx.arc(center.y, center.x, d, arcStart-Math.PI/8, arcStart-Math.PI/8 + Math.PI/4, false);
    cache.ctx.lineTo(center.y, center.x);
    cache.ctx.closePath();
    cache.ctx.fill();
  ...
  light will be rendered.
  **/
  cp.Lamp.prototype.render = function (ctx, arcStart) {
    var center = this.center();
    var c = this._getGradientCache(center, arcStart);
  ...
  **/
  cp.Lighting.prototype.compute = function (w,h,arcStart) {
  ...
  **/
  ...
  /**
  Compute the dark mask.f
  @method compute
  @param {Number} w Width of the canvas context.
  @param {Number} h Height of the canvas context.
  **/
  cp.DarkMask.prototype.compute = function (w,h, arcStart, objects) {
    if (!this._cache || this._cache.w != w || this._cache.h != h)
    ...
    this.lights.forEach(function(light){
      light.mask(ctx, arcStart, objects);
    });
    ctx.restore();
  }
